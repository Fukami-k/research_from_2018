...
#settings
#################
load("../produced_data/Rdata/crop_calendar.Maize.RData")
order2 <- "maize"
...
for(l in 1:length(climateterm)){
    climateGD <- data.frame(year = rep(yearlist, each = countrylength), COWN = rep(1:countrylength, times = yearlength),
                            value = rep(NA, times = countrylength * yearlength))
    climateData<-rep(NA,720*360*120)

    climateData.nc <- open.nc(paste0("../rawdata/CRU_2017/cru_ts3.24.", as.character(1 + min(yearlist-1) %/%10*10),
    ...
    for(yearc in yearlist){
        if(yearc%%10 == 1){
            if(yearc%/%10 <= 200){
                climateData.nc <- open.nc(paste0("../rawdata/CRU_2017/cru_ts3.24.", as.character(1+yearc%/%10*10),
                    ".", as.character(10+yearc%/%10*10), ".", climateterm[l], ".dat.nc"))
                climateData <- var.get.nc(climateData.nc, climateterm[l])
            }else{
                climateData.nc <- open.nc(paste0("../rawdata/CRU_2017/cru_ts3.24.2011.2015.", climateterm[l], ".dat.nc"))
                climateData <- var.get.nc(climateData.nc,climateterm[l])
            }
        }
